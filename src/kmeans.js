...
export function kMeans(img, k) {
  var pixel_dataset = get_pixel_dataset(img, MAX_K_MEANS_PIXELS);
  var centroids = k_means(pixel_dataset, k);
  var quantizedCanvas = quantize(img, centroids);
  return quantizedCanvas;
}

function findMostCommonColor(data, nChannels, width, fromX, toX, fromY, toY) {
  let cellColors = {};
...
export function quantizeKMeans(img, k = 1) {
  let kMeansImage = kMeans(img, k);
  let ctx = kMeansImage.getContext('2d');
  let width = kMeansImage.width;
  let height = kMeansImage.height;

  let data = ctx.getImageData(0, 0, width, height).data;
  let nPixels = width * height;
  let nChannels = data.length / nPixels;

  let color = findMostCommonColor(data, nChannels, width, 0, width, 0, height);
